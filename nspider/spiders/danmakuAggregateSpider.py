...
from nspider.items import DanmakuAggregateItem
...
class DanmakuAggregateSpider(RedisSpider):
    ...
    custom_settings = {
        'DOWNLOAD_DELAY': 1,
        'ITEM_PIPELINES': {
            'nspider.pipelines.DanmakuAggregatePipeline': 300
        },
    }
    ...
    def parse(self, response):
        try:
            ...
            self.errorCollection.insert_one({'aid': int(aid), 'url': response.url, 'error': error})
    ...
    def getCidPlanB(self, response):
        try:
            ...
            yield Request(self.DANMAKU_API.format(oid=cid), callback=self.parseDanmaku,
                          meta={'object_id': object_id, 'duration': duration, 'p_name': '', 'page_number': 1,
                                'aid': int(aid)})
        except Exception as error:
            self.errorCollection.insert_one({'aid': int(aid), 'url': response.url, 'error': error})
    ...
    def parseDanmaku(self, response):
        try:
            ...
            # 全角转半角，转大写
            danmaku_text = self.q_to_b(" ".join(response.xpath("d/text()").extract()).upper())
            ...
            # 计算弹幕密度
            danmaku_attr = list(map(lambda x: x.split(","), response.xpath("d/@p").extract()))
            tick = duration / 50
            ...
            self.errorCollection.insert_one({'aid': int(response.meta['aid']), 'url': response.url, 'error': error})
